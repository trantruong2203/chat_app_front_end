...
const AddFriendRequest: React.FC = () => {
    ...
    const handleRejectFriend = async (id: number) => {
        try {
            const request: FriendShip | undefined = friendRequests.find(req => req.id == id);
            if (!request) {
                toast.error('Lời mời kết bạn không tồn tại');
                return;
            }
            await dispatch(deletedFriendShip(request.id));
    ...
    return (
        ...
        >
                ...
                {friendRequests.length > 0 ? (
                    <List
                        ...
                        renderItem={(request) => {
                            ...
                            console.log(request, senderUser);
            ...
            <div style={{ padding: '0 10px' }}>
                <Typography.Title level={4} style={{ margin: '15px 0', color: '#333' }}>
                    Lời mời đã gửi ({friendRequestsSent.length})
                </Typography.Title>
